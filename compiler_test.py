...
class CompiledExecutable(RedirectedExecutable):
    ...
    def __init__(self, path, dir, library_dirs=[], interpreter=None):
        """Construct a new 'CompiledExecutable'.

        'path' -- A string giving the location of the executable.

        'dir' -- The directory in which to run the executable.
        
        'library_dirs' -- A sequence of directories that should be
        searched for shared libraries when the program is run.

        'interpreter' -- If not 'None', the path to an interpreter
        that should be used to execute the program.  (For example,
        when testing a cross-compiler, the 'interpreter' might be the
        path to a simulator.)"""

        # If there is an interpreter, save away the path to the
        # compiled executable.  The program that will really be
        # executed is the interpreter.
        if interpreter:
            self.__saved_path = path
            path = interpreter
        else:
            self.__saved_path = None
        # Initialize the base class.
        RedirectedExecutable.__init__(self, path, dir)
    ...
    def Run(self, arguments=[]):
        ...
        # If there's an interpreter, we saved away the path to the
        # compiled program.  Add it back now.
        if self.__saved_path:
            arguments = [self.__saved_path] + arguments

        return RedirectedExecutable.Run(self, arguments)
